...
exports.memberJoinMeetup = (req, res, next) => {
  const meetupId = req.body.meetupId
  const userId = req.body.userId

  const join = new MembersModel({
    meetup: meetupId,
    user: userId
  })

  join.save((err, member) => {
    if (err) return ErrorHandler.response(err, req, res, next)
...
exports.members = (req, res, next) => {
  ...
  MembersModel
    .find({ meetup: meetupId })
    .populate('user')
    .exec((err, listMembers) => {
      if (err) return ErrorHandler.response(err, req, res, next)

      return res.status(httpStatus.OK).json(listMembers)
    })
}
