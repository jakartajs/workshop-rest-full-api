...
describe('Meetups API', () => {
  ...
  it('POST /v1/meetups/members/joins -> should return 201 CREATED', (done) => {
    const data = {
    ...
    server
      ...
      .end((err, res) => {
        ...
        res.body.meetup.should.equal(data.meetupId)
        res.body.user.should.equal(data.userId)

        global.currentMember = res.body
        done()
      })
  })

  it('GET /v1/meetups/:meetupId/members -> should return 200 OK', (done) => {
    const data = {
      userId: currentUser._id,
      meetupId: currentMeetup._id
    }

    const endpoint = `/v1/meetups/${currentMeetup._id}/members`
